...
class BaseTD:
    def __init__(self, task: BaseTask, **kwargs):
        ...
        if self.task.__class__.__name__ == 'BairdCounterExample':
            self.w = np.ones(self.task.num_features)
            self.w[7] = 10
    ...
    def compute_rmsve(self):
        off_policy_error_tasks = []
        if self.task.__class__.__name__ in off_policy_error_tasks:
            # FIXME: this is too ad-hoc
            error_dist = self.d_mu
        else:
            error_dist = self.task.target_dist
        error = self.compute_value_function() - self.state_values
        error_squared = error * error
        return np.sqrt(np.sum(error_dist * error_squared.T, 0) / np.sum(error_dist, 0)), error
    ...
    def choose_target_action(self, policy_id=0):
        return self.task.select_target_action(self.state, policy_id)
